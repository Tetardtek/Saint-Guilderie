  ...
  try {
    ...
    return res.status(200).json({
      ...
      user: {
        ...user,
        pseudo: user.pseudo,
        rolename: user.id_roles.toString(),
      },
      token,
    });
  } catch (error) {
...
const edit = async (req, res) => {
  ...
  try {
    ...
    const { pseudo, mail, corn, id_roles, currentPassword, newPassword } =
      req.body;
    ...
    if (corn !== undefined) {
      updatedFields.corn = corn;
    }

    if (id_roles !== undefined) {
      updatedFields.id_roles = id_roles;
    }
...
const add = async (req, res, next) => {
  try {
    const { pseudo, password, mail, corn, id_roles } = req.body;
